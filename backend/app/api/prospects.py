    ...
    try:
        ...
        try:
            ...
            if not has_final_body:
                ...
                logger.info("🔧 [SCRAPED EMAILS] final_body column missing - using raw SQL workaround")
                raw_query = text(f"""
                ...
                for row in rows:
                    ...
                    p.updated_at = row[37]
                    prospects.append(p)
                logger.info(f"🔍 [SCRAPED EMAILS] Found {len(prospects)} prospects using raw SQL workaround")
            else:
