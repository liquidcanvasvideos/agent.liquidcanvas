...
async def list_profiles(
    ...
    """
    try:
        logger.info(f"📊 [SOCIAL PROFILES] Request: skip={skip}, limit={limit}, platform={platform}, discovery_status={discovery_status}, category={category}, username={username}")
        ...
        if category:
            query = query.where(func.lower(Prospect.discovery_category) == category.lower())
            count_query = count_query.where(func.lower(Prospect.discovery_category) == category.lower())
            
        if username:
            query = query.where(Prospect.username.ilike(f"%{username}%"))
            count_query = count_query.where(Prospect.username.ilike(f"%{username}%"))
...
async def send_messages(
    ...
    task = asyncio.create_task(process_social_send_job(str(job.id)))
    register_task(str(job.id), task)
...
async def list_drafted_profiles(
    ...
    try:
        ...
        if location:
            query = query.where(func.lower(Prospect.discovery_location) == location.lower())
            count_query = count_query.where(func.lower(Prospect.discovery_location) == location.lower())
...
async def list_sent_profiles(
    ...
    REUSES prospects table - filters by source_type='social' AND (send_status='sent' OR outreach_status='sent').
    """
    try:
        # SCHEMA MUST BE CORRECT - migrations run on startup ensure all columns exist
        # If source_type column doesn't exist, queries will fail loudly with HTTP 500
        ...
        # NOTE: social sending code historically updated outreach_status; newer code may update send_status.
        # Accept either as "sent" so the Sent tab reflects reality.
        sent_filter = and_(
            Prospect.source_type == 'social',
            or_(
        ...
        query = select(Prospect).where(sent_filter)
        count_query = select(func.count(Prospect.id)).where(sent_filter)
