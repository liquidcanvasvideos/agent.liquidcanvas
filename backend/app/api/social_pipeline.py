...
async def get_social_pipeline_status(
    ...
    try:
        ...
        if not column_exists:
            return {
                "discovered": 0, "reviewed": 0, "qualified": 0, "drafted": 0, "sent": 0, "followup_ready": 0,
                "status": "inactive", "message": "Social outreach columns not initialized."
            }
        ...
        if platform:
            social_filter = and_(social_filter, Prospect.source_platform == platform.lower())
            
        if category:
            social_filter = and_(social_filter, func.lower(Prospect.discovery_category) == category.lower())
...
async def send_messages(
    ...
    try:
        ...
        try:
            ...
            register_task(str(job.id), task)
            logger.info(f"✅ [SOCIAL PIPELINE STAGE 4] Social send job {job.id} started in background")
        except Exception as e:
            logger.error(f"❌ [SOCIAL PIPELINE STAGE 4] Failed to start social send job: {e}", exc_info=True)
            job.status = "failed"
            job.error_message = str(e)
            await db.commit()
