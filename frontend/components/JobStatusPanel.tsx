...
export default function JobStatusPanel({ jobs, expanded = false, onRefresh }: JobStatusPanelProps) {
  ...
  const renderSocialSendJobDetails = (job: Job) => {
    if (job.job_type !== 'social_send' || !job.result) return null
      ...
      <div className="mt-3 space-y-3 pt-3 border-t border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className="glass rounded-xl p-2 border border-green-200/50 shadow-sm">
            <div className="text-xs text-green-700 font-semibold mb-1">Sent</div>
            <div className="text-lg font-bold text-green-800">{result.sent || 0}</div>
          </div>
          <div className="glass rounded-xl p-2 border border-red-200/50 shadow-sm">
            ...
            <div className="text-lg font-bold text-red-800">{result.failed || 0}</div>
          </div>
          <div className="glass rounded-xl p-2 border border-blue-200/50 shadow-sm">
            ...
            <div className="text-lg font-bold text-blue-800">{result.total || 0}</div>
          </div>
          <div className="glass rounded-xl p-2 border border-amber-200/50 shadow-sm">
            <div className="text-xs text-amber-700 font-semibold mb-1">Current</div>
            <div className="text-lg font-bold text-amber-800">{result.current || 0}</div>
          </div>
        </div>

        <div className="glass rounded-xl p-3 border border-gray-200/50 shadow-sm space-y-2 text-xs">
          <div>
            <span className="text-gray-600">Current action:</span>
            <span className="ml-1 font-semibold text-gray-900">{result.current_action || '—'}</span>
          </div>
          {(result.current_username || result.current_platform) && (
            <div>
              <span className="text-gray-600">Target:</span>
              <span className="ml-1 font-semibold text-gray-900">
                {result.current_username ? `@${result.current_username}` : '—'}
                {result.current_platform ? ` on ${result.current_platform}` : ''}
              </span>
            </div>
          )}
          {result.last_error && (
            <div className="text-red-700">
              <span className="text-red-700 font-semibold">Last error:</span>
              <span className="ml-1">{result.last_error}</span>
            </div>
          )}
        </div>
        ...
        {events.length > 0 && (
          <div className="bg-gray-50 rounded-lg p-3">
            <div className="text-xs text-gray-700 font-semibold mb-2">Live events</div>
              ...
              >
                ...
                {isExpanded && (
                  <>
                    ...
                    {renderSocialSendJobDetails(job)}
                  </>
                )}
