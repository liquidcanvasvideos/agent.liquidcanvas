...
export default function SocialDraftsTable() {
  ...
  const loadDrafts = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await listSocialDrafts(
        skip, 
        limit, 
        undefined,
  ...
  }, [skip, searchCategory, searchUsername, searchLocation])
  ...
  return (
    <div className="glass rounded-xl shadow-lg p-4 animate-fade-in">
      ...
      <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
        ...
        <div>
          <label className="block text-[10px] font-bold text-olive-700 uppercase mb-1">Location</label>
          <select
