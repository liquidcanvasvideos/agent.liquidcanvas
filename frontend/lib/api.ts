...
export async function uploadProspectAttachment(params: {
  prospectId: string
  file: File
}): Promise<UploadAttachmentResponse> {
  const form = new FormData()
  form.append('scope', 'prospect')
  form.append('prospect_id', params.prospectId)
...
export async function generateCategoryTemplate(
  request: PipelineCategoryTemplateRequest
): Promise<PipelineCategoryTemplateResponse> {
  const res = await authenticatedFetch(`${API_BASE}/pipeline/draft/category/template`, {
    method: 'POST',
    body: JSON.stringify(request),
  })
  if (!res.ok) {
    const error = await res.json().catch(() => ({ detail: 'Failed to generate category template' }))
    throw new Error(error.detail || 'Failed to generate category template')
  }
  return res.json()
}

export async function pipelineDraftCategory(
  request: PipelineDraftCategoryRequest
): Promise<PipelineDraftResponse> {
  const res = await authenticatedFetch(`${API_BASE}/pipeline/draft/category`, {
    method: 'POST',
    body: JSON.stringify(request),
  })
  if (!res.ok) {
    const error = await res.json().catch(() => ({ detail: 'Failed to start category drafting' }))
    const errorObj: any = new Error(error.detail || 'Failed to start category drafting')
...
export async function getProspectSentEmail(prospectId: string): Promise<SentEmailDetails> {
  const res = await authenticatedFetch(`${API_BASE}/prospects/${prospectId}/sent-email`)
  if (!res.ok) {
    const error = await res.json().catch(() => ({ detail: 'Failed to get sent email' }))
    throw new Error(error.detail || 'Failed to get sent email')
  }
  return res.json()
}
...
export async function sendEmail(
  ...
  const hasPayload = !!(options && (options.cc?.trim() || options.bcc?.trim()))
    ...
    ...(hasPayload
      ? {
          body: JSON.stringify({
            ...
            bcc: options?.bcc?.trim() || undefined,
          }),
        }
      : {}),
  })
...
export interface PipelineDraftCategoryFilters {
  category?: string
}

export interface PipelineCategoryTemplateRequest {
  category: string
  notes?: string
}

export interface PipelineCategoryTemplateResponse {
  success: boolean
  category: string
  concept: string
...
export interface PipelineDraftCategoryRequest {
  prospect_ids?: string[]
  filters?: PipelineDraftCategoryFilters
  category: string
  concept: string
  subject_template: string
  body_template: string
...
export async function listWebsites(skip: number = 0, limit: number = 50, category?: string): Promise<{
  data: Array<{
    id: string
    domain: string
    url: string
  ...
  if (category && category !== 'all') params.append('category', category)
  const res = await authenticatedFetch(`${API_BASE}/pipeline/websites?${params}`)
  if (!res.ok) {
